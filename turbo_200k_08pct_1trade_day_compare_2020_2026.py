...
def simulate(buffer):
    ...
    bal = START
    floor = START - TRAIL  # user's custom fixed 6% floor before first payout on each replacement
    acct = 1
    cycle_start = None
    cycle_base = START
    ...
    for _, r in tr.iterrows():
        ...
        bal = desired_after
        floor = START  # payout locks floor at initial balance
        cycle_start = day
...
pd.DataFrame(summaries).to_csv('turbo_compare_2020_2026_summary.csv', index=False)
pd.DataFrame(all_yearly).to_csv('turbo_compare_2020_2026_yearly.csv', index=False)
pd.DataFrame(all_payouts).to_csv('turbo_compare_2020_2026_payouts.csv', index=False)
pd.DataFrame(all_breaches).to_csv('turbo_compare_2020_2026_breaches.csv', index=False)
