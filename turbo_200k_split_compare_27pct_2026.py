...
CONFIGS=[
    {'name':'1x200k','n':1,'size':200000.0,'cost':1098.0},
    {'name':'2x100k','n':2,'size':100000.0,'cost':549.0},
    {'name':'4x50k','n':4,'size':50000.0,'cost':349.0},
]
...
for cfg in CONFIGS:
    ...
    for i in range(n):
        states.append({
            ...
            'daily':defaultdict(float),'accounts_used':1,
    ...
    summary.append({
        ...
        'account_cost_usd':account_cost,'net_cash_usd':net,
        ...
        'first_portfolio_payout_days_from_2026_start':(portfolio_paydates[0]-pd.Timestamp('2026-01-01')).days if portfolio_paydates else np.nan
    })
...
out.to_csv('turbo_200k_split_compare_27pct_2026_summary.csv',index=False)
pd.DataFrame(slot_rows).to_csv('turbo_200k_split_compare_27pct_2026_slots.csv',index=False)
