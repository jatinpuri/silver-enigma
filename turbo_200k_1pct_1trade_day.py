...
# Hard one-triggered-trade-per-trading-day rule.
# Choose the first qualifying/triggered setup chronologically for each signal date;
# later qualifying windows that same day are skipped.
...
def sim(mode):
    ...
    for _,r in tr.iterrows():
        # Daily P&L is tracked by the trade's signal/trading day so the hard one-trade/day
        # rule cannot create >1 trade of P&L in a cycle day.
        day=r.trade_day
        ...
        # custom: fixed 6% floor until payout.
...
pd.DataFrame(rows).to_csv('turbo_200k_1pct_1trade_day_summary.csv',index=False)
pd.DataFrame(pays).to_csv('turbo_200k_1pct_1trade_day_payouts.csv',index=False)
pd.DataFrame(br).to_csv('turbo_200k_1pct_1trade_day_breaches.csv',index=False)
pd.DataFrame(trades_used,columns=['trade_day','window','R','pnl','exit_dt']).to_csv('turbo_200k_1pct_1trade_day_trades.csv',index=False)
