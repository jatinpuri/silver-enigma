...
tr=tr[(tr.year==YEAR)&(tr.window.isin(WINDOWS))&(tr.local_weekday<=4)].copy()
...
def sim(mode):
    ...
    acct=1; cycle_start=None; cycle_base=START; daily=defaultdict(float)
    ...
    for _,r in tr.iterrows():
        ...
        used.append((str(day),acct,r.window,float(r.R),pnl,bal,floor,str(r.exit_dt)))
        ...
        gross_request=max(0.0, bal-desired_after)
        if gross_request<=0: continue
        user=gross_request*SPLIT
        payouts.append((acct,str(day),gross_request,user,profdays,best,cons,bal,floor,desired_after))
        bal=desired_after
        floor=START
        high=max(high,bal)
        locked=True
    ...
    cons=best/cycle_profit if cycle_profit>0 else np.inf
    return {
      'mode':mode,'year':YEAR,'trades':len(tr),'payouts':len(payouts),'breaches':len(breaches),'accounts_used':acct,
      'gross_requested':sum(x[2] for x in payouts),'user_paid':sum(x[3] for x in payouts),
      'ending_balance':bal,'ending_floor':floor,'ending_cycle_base':cycle_base,'ending_cycle_profit':cycle_profit,
...
for mode in ['custom','official']:
    ...
    for x in p:
        pays.append({'mode':mode,'account':x[0],'date':x[1],'gross_requested':x[2],'user_payout':x[3],
                     'profitable_days':x[4],'best_day':x[5],'consistency':x[6],'balance_before':x[7],
                     'floor_before':x[8],'balance_after':x[9]})
    for x in b:
        br.append({'mode':mode,'account':x[0],'date':x[1],'balance':x[2],'floor':x[3],'pnl':x[4],'window':x[5]})

pd.DataFrame(rows).to_csv('turbo_200k_08pct_1trade_day_buffer4k_2025_summary.csv',index=False)
pd.DataFrame(pays).to_csv('turbo_200k_08pct_1trade_day_buffer4k_2025_payouts.csv',index=False)
pd.DataFrame(br).to_csv('turbo_200k_08pct_1trade_day_buffer4k_2025_breaches.csv',index=False)
pd.DataFrame(trades_used,columns=['trade_day','account','window','R','pnl','balance_after_trade','floor_before','exit_dt']).to_csv('turbo_200k_08pct_1trade_day_buffer4k_2025_trades.csv',index=False)
