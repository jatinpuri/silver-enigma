...
def sim(mode):
    ...
    for _,r in tr.iterrows():
        ...
        # On first payout, retain a $2k buffer. On later payouts, preserve the same $202k base.
...
pd.DataFrame(rows).to_csv('turbo_200k_08pct_1trade_day_buffer2k_summary.csv',index=False)
pd.DataFrame(pays).to_csv('turbo_200k_08pct_1trade_day_buffer2k_payouts.csv',index=False)
pd.DataFrame(br).to_csv('turbo_200k_08pct_1trade_day_buffer2k_breaches.csv',index=False)
