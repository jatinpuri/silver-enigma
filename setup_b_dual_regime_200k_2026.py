...
for i in idxs:
    ...
    # Dual-regime rule:
    # rising ADX -> trade with candle; falling ADX -> trade opposite candle.
    if r.adx>p.adx:
        d=c; regime='ADX_RISE_WITH_CANDLE'
    elif r.adx<p.adx:
        d=-c; regime='ADX_FALL_REVERSE_CANDLE'
    else:
        continue
    j=i+1
    ...
    stop=entry-d*stop_dist; target=entry+d*TP_ATR*float(r.atr)
    ...
    trades.append(dict(signal_dt=r['dt'],entry_dt=df.loc[j,'dt'],exit_dt=df.loc[exit_idx,'dt'],trade_day=r['dt'].date(),R=float(rv),stop_pips=stop_pips,regime=regime))
...
tr.to_csv('setup_b_dual_regime_2026_trades.csv',index=False)
...
for label,stress in [('COMMISSION',False),('COMM_PLUS_0P2',True)]:
    for rp in RISK_PCTS:
        rows.append(dict(cost_model=label,**simulate(rp,stress)))
out=pd.DataFrame(rows); out.to_csv('setup_b_dual_regime_200k_2026_results.csv',index=False)
summary=pd.DataFrame([{
    'trades':len(tr),'rise_trades':int((tr.regime=='ADX_RISE_WITH_CANDLE').sum()),'fall_reverse_trades':int((tr.regime=='ADX_FALL_REVERSE_CANDLE').sum()),
    'win_rate':float((tr.R>0).mean()),'EV_R':float(tr.R.mean()),'total_R':float(tr.R.sum())
}]); summary.to_csv('setup_b_dual_regime_2026_summary.csv',index=False)
